...
const EditEmployee = () => {
        ...
        const fetchEmployee = async () => {
            try {
                ...
                setFormData({
                    ...
                    residence_landline: emp.residence_landline || emp.mobile || emp.phone || '',
                    division_unit: emp.division_unit || '',
                    address: emp.address || '',
                    residence_postal_code: emp.residence_postal_code || emp.pincode || '',
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach(key => {
            if (key === 'password' && !formData[key]) return; // Skip password if empty in edit
        ...
        try {
            ...
            alert(error.response?.data?.error || error.response?.data?.message || 'Failed to update employee');
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-[var(--text-secondary)] mt-1">
                                Update employee details for {formData.first_name}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 rounded-3xl bg-[var(--surface-card)] border border-[var(--border-default)] shadow-xl animate-fade-in relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
                        ...
                        <InputField label="Employee Code" name="emp_code" value={formData.emp_code} onChange={handleChange} required placeholder="e.g. 10009" error={errors.emp_code} disabled />
                        ...
                        <InputField label="Password" name="password" type={showPassword ? "text" : "password"} value={formData.password} onChange={handleChange} placeholder="Leave blank to keep same" error={errors.password} showToggle onToggle={() => setShowPassword(!showPassword)} isVisible={showPassword} />
